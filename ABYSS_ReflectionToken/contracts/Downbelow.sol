...
contract Downbelow is Pausable {
    ...
    uint256 private creditTracking = 0; // just example value
    ...
    function checkTotalCredits(uint256 totalCredits, uint256 dailyPayouts, uint256 deadline,
        ...
        require(block.timestamp <= deadline, "Downbelow: INVALID_EXPIRATION IN CHECK");
        require(_msgSender() == downbelowSigner, "Downbelow: YOU ARE NOT DOWNBELOW_SIGNER");
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                '\x19\x01',
                DOMAIN_SEPARATOR,
                keccak256(abi.encode(DOWNBELOW_TYPEHASH, _msgSender(), dailyPayouts, totalCredits, currentValidNonce, deadline))
            )
        );

        require(downbelowSigner == ecrecover(digest, v, r, s), "Downbelow: INVALID_SIGNATURE IN CHECK");
        _nonces[_msgSender()] = currentValidNonce + 1;
        
        creditTracking = creditTracking + dailyPayouts;
        checkId = checkId + 1;
        if(totalCredits != creditTracking)  // set Pause
            _pause();

        emit Checked(checkId, _msgSender(), totalCredits, creditTracking, totalCredits == creditTracking, getCurrentTime());
    }
}